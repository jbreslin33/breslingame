...
void CArmyWar::AddClient(int local, int ind, char *name)
{
	...
	if(clientList == NULL)
	{
	    ...
	    clientList->character = new SinbadCharacterController(mBaseGame->getCamera(), "Sinbad" + ind);
		if(clients % 2 == 0)
	...
	//	list->character = new SinbadCharacterController(mBaseGame->getCamera());
	    list->character = new SinbadCharacterController(mBaseGame->getCamera(), "Sinbad" + ind);
	}
